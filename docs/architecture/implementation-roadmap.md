...
Timeline completion update: the desktop store now models local pending and failed text/attachment sends, renders group read receipts as per-member detail (`read by u_bob` / `read by all`), feeds a custom Qt `BubbleListView` / `BubbleDelegate` painter used by the Qt UI, and shows legacy messages with an explicit `legacy` timestamp fallback instead of an ambiguous missing time.
...
Telegram reference detail update: the desktop shell now also carries Telegram-style date/service chips, bottom info metadata, reaction affordances, left folder tabs, side-menu extension points, send-as/send-options composer controls and photo/file drag-drop zones while keeping locked reference screenshots stable through GUI smoke image diff.
