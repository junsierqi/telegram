...
Last synced: after milestone **M65 — dev cert generation and live TLS proxy smoke**.
Bundle status: PostgreSQL repository validator 3/3, PostgreSQL backup/restore validator 1/1, session hardening 3/3, TLS config 3/3, TLS handshake 1/1, TLS deployment config 2/2, TLS dev cert 2/2, live TLS proxy smoke passed, Windows package validation passed with zip + SHA256.
...
| `net/tcp_line_client.h/.cpp` | **A1.** Async JSON-line TCP client. Background reader thread drains `\n`-delimited frames into a `std::deque<std::string>` behind mutex + condvar. `send_line()` is thread-safe via a separate write mutex. `wait_for(predicate, timeout_ms)` pops the first matching frame, leaves the rest in queue order. Predicate builders: `match_correlation_id(id)` (naive JSON scan), `match_any()`. Destructor `shutdown(SD_BOTH / SHUT_RDWR)` to unblock `recv()`, then joins the reader. |
...
| `validate_tls_deployment_config.py` | Static checks for the Docker `nginx` TLS termination profile and proxy config. |
...
| `validate_tls_proxy_smoke.py` | Performs a TLS login request through the Docker nginx stream proxy on port 8443. |
...
| TLS/client transport parity | The Python TCP control plane can run native TLS with configured PEM material and has a real TLS login smoke. Docker includes an `nginx` stream TLS termination profile that listens on 8443 and forwards to the plaintext server; a dev cert generator and live proxy smoke are available. Existing C++ clients still speak plaintext, so direct desktop-to-TLS requires a C++ TLS transport slice next. |
