...
| Product-grade desktop UI polish | Telegram-style 3-pane shell + 8-page Settings + Remote + Call panels + design-token palette + real image preview (M125) + byte-level upload progress (M126) ship. Remaining polish: rich message-action cards/pickers, advanced conflict recovery metadata, CSS-style design system applied to every QSS literal (currently 4 of ~30 tokens are referenced; rest are ready for future tweaks). |
