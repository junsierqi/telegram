...
| File | Purpose |
|---|---|
| `server/server/protocol.py` | Single source of truth for the wire format. `MessageType` StrEnum (~40 values — login, register, message_send, service_command, message_forward, message_reaction, message_pin, message_read, message_edit, conversation_create, heartbeat_ping, presence_query_request, contact_*, attachment_*, remote_*, …). `ErrorCode` StrEnum (~40 codes) with matching human messages. Typed `@dataclass(slots=True)` payloads for every request and response, including message `created_at_ms`, reply/forward metadata, service-command payloads, reaction summary, pinned state, silent/scheduled send flags, attachment filename/mime/size metadata on synced messages, conversation-level pinned/archived/muted state, `ConversationSyncRequestPayload(cursors=..., versions=..., history_limits=..., before_message_ids=...)`, history-page `next_before_message_id` / `has_more`, message-search `offset` / `next_offset` / `has_more`, tdesktop-parity contact edit/share, report reason, leave confirmation, shared-media page payloads, account settings payloads and account feature payloads, and `ConversationChangeDescriptor` for incremental sync. `parse_request` dispatches to per-type parsers. `Envelope` / `RequestMessage` / `ResponseMessage` structs; `make_envelope`, `make_response`, `ServiceError`. |
| `server/server/state.py` | **`InMemoryState`** holds `users`, `devices`, `sessions`, `conversations`, `remote_sessions`, `contacts`, `contact_aliases`, `conversation_reports`, `account_settings`, `account_features`, `attachments`. Conversation records now include a durable `change_seq` and change log for edit/delete/read-marker deltas. It supports JSON `--state-file`, SQLite `--db-file`, and PostgreSQL `--pg-dsn` through `PostgresStateRepository`. `save_runtime_state()` writes the configured backend; `_load_runtime_state()` is defensive for JSON and schema-aware for SQLite/PostgreSQL. `sessions_for_user(uid)` is the fan-out lookup helper. |
...
| `services/contacts.py` | Directed per-user contact lists keyed by owner_user_id. `add` rejects self-adds + duplicates + unknown targets. `edit` stores per-owner display aliases, `share` returns a Telegram-style share string, `remove` clears the alias, and `list` returns descriptors enriched with live presence. |
...
| `transport/control_plane_client.h/.cpp` | **A2/C3/C5/C6/M52/M54/M55/M56.** High-level typed wrapper around `TcpLineClient`. Every RPC assigns a fresh `rpc_<n>` correlation_id and blocks on the exact match. Typed results with `{ok, error_code, error_message}` for: `login`, `register_user`, `profile_get`, `profile_update`, `search_users`, `search_messages`, `conversation_sync`, `conversation_sync_since`, `conversation_history_page`, `send_message`, `reply_message`, `forward_message`, `toggle_reaction`, `set_message_pin`, `send_attachment`, `fetch_attachment`, `mark_read`, `edit_message`, `delete_message`, `list_contacts` / `add_contact` / `edit_contact` / `share_contact` / `remove_contact`, `account_settings_get` / `account_settings_update`, `account_features_get` / `account_features_update`, `create_conversation` / `add_conversation_participant` / `remove_conversation_participant` / `leave_conversation`, `report_conversation`, `shared_media_page`, `list_devices`, `revoke_device`, `update_device_trust`, `presence_query`, `heartbeat_ping`. Message responses parse `created_at_ms`, reply/forward metadata, reaction summary and pinned state; attachment sync responses parse attachment_id plus filename/mime/size; message-search and shared-media responses parse offset paging flags. Incremental sync cursors carry both `last_message_id` and `version`, history pages carry `next_before_message_id` / `has_more`, and responses parse `changes` plus read markers. A background **dispatcher thread** pulls any frame whose correlation_id starts with `push_` and invokes a single `PushHandler` callback (`type`, `envelope_json`). Optional `start_heartbeat(interval_ms)` spawns a periodic `heartbeat_ping` thread. |
...
| `client/src/app_desktop/main.cpp` | **C1-C6c/C3e + registration/contacts/groups/timeline/profile/search/message-actions UI.** Qt Widgets desktop baseline. Provides host/port/conversation fields, login/register form, profile load/update controls, user discovery search with online/contact flags, chat-list filtering, All/Unread/Pinned/Archived folder tabs, selected-conversation message search with prev/next navigation, server-side search result cards, older-history loading for the selected conversation, left-side conversation list, rich read-only message timeline, composer, live push display, text send, reply/forward/reaction/pin controls, an Attach button, attachment-id save control, stage-based attachment transfer status, contacts add/remove/list controls, group create/member controls and a device-management panel. Device panel lists active/trusted devices, confirms destructive revoke, and can revoke non-current devices or toggle trust. Message rows render through the custom `BubbleListView` / `BubbleDelegate` painter pipeline as Telegram-style inbound/outbound bubbles with timestamps, pending/sent/failed/read status, per-member group read details, reply/forward/reaction/pin metadata, service/date chips, media/document surfaces, image time overlays, reaction affordances, bottom-info metadata (`views` / `comments` / `forwards` / edited / tick) and search match/focused highlighting. Hovering a bubble exposes a Telegram-style action button that opens the same message menu as right-click; that menu now routes quick reaction selection, attachment save/copy actions, reply/forward/pin/delete and inline edit. The composer exposes reply/edit headers, inline edit submission through the message send path, a compact composer status line, retained send-as identity UI state, silent/scheduled send options, emoji/sticker preview with recent token tracking, voice-as-audio attachment state and multi-file photo/video/audio/file drag-drop routing while keeping legacy power-user controls hidden by default. The left chat list exposes real pin/archive/mute and mark-as-read quick actions backed by control-plane RPCs, folder route shortcuts from the row menu, and live All/Unread/Pinned/Archived tab counts. The hamburger drawer routes profile/account, Wallet/Premium/Stories feature state, Emoji Status, account switching, Archived Chats with count plus context menu, Calls, Saved Messages as the current user self-peer, Cloud Storage account-export summary, Reset Scale and a custom Night Mode toggle to real local/server state; the Night Mode toggle applies the theme immediately without the old hidden-checkbox bridge, and the drawer footer lives in the scrollable menu content and resizes with the window like tdesktop's main menu footer. Attachment rows show filename/size metadata, type previews and the save dialog defaults to the known filename. The Telegram Desktop reference shell includes the collapsible right details panel; reference states for channel, one-to-one contact, bot/service and group info render through explicit `DetailPeerKind` classification (`User`, `Bot`, `Service`, `BasicGroup`, `MegaGroup`, `BroadcastChannel`) and separate peer-specific branches so `reference-09-chat-channel-info-panel.png`, `reference-10-chat-user-info-panel.png` and `reference-11-chat-group-info-panel-empty.png` can keep their distinct titles, subtitles, actions, media rows and member/contact sections without leaking group/channel danger actions into one-to-one profiles; one-to-one user info stays as a direct profile surface with a real notifications toggle, media counts and contact actions instead of extra right-column navigation rows. The right details panel now opens/closes through a tdesktop-style right-column helper that tries to extend or shrink the window width outside locked screenshot mode. Service and bot conversations no longer share the same peer kind: official service conversations use notification/security rows, while bot conversations expose command/add-to-group style rows. Official-service rows call the backend `service_command` RPC for `/start`, `/help` and `/security`; slash commands typed directly into the composer take the same service-command path instead of becoming plain chat messages, and typing `/` opens a Telegram-style command suggestion popup for the supported service commands. Shared media/files/links/voice rows are exposed as Telegram-style profile rows or drill-in surfaces rather than visible Qt tab controls; the underlying rows carry message/attachment/link metadata so clicking can focus a message, select an attachment id for saving, or copy a shared link, and their right-click menu exposes focus/copy/save actions while Load-more rows call the server `shared_media_page` provider. Settings General modal rows route into the real settings stack pages for Appearance, Connection, Privacy, Chat Tools, Devices, Contacts and Advanced, plus Proxy opens the proxy settings dialog; Privacy and Connection pages can persist account settings through the server-backed notification/privacy/security/proxy model. The Contacts modal now behaves closer to tdesktop `peer_list_box`: selected rows support edit alias, share-to-clipboard and delete through typed contact RPCs. The Profile modal uses a compact info/profile box shape with top edit/close buttons, avatar header and form-like read-only fields, and its Stories area publishes through the account-features RPC. The right details panel uses real data for shared media/files/links/voice rows, filters group members from `participant_user_ids`, derives profile fields from the selected conversation/user id/message counts instead of fixed screenshot strings, lets member clicks open the Contacts action page for that user, and routes visible action buttons to message focus, mute/unmute, chat info, service/bot command/settings/add-group rows, contact share/edit/delete, block user, backend Gift recording, report-reason submission and confirmed leave. Visible drawer/modal/right-panel buttons now have explicit response coverage: profile edit opens Profile settings, contacts sort toggles A-Z/Z-A, create/proxy overflow menus open concrete routes, proxy share copies a link, FAQ copies the Telegram FAQ URL, and Gift/Wallet/Stories/Emoji Status call account-domain RPCs. Includes `--smoke` / `--smoke-register` / `--smoke-attachment` / `--gui-smoke` / `--smoke-save-dir` mode for non-interactive validation of connect → login/register → local navigation/search → server message search → profile get/update → user search → sync → text send → reply/forward/reaction/pin → incremental sync → device list → contacts add/remove → group create → attachment send/fetch/save plus transfer-status markers and locked desktop reference screenshots. The UI renders from `DesktopChatStore`, loads `--cache-file` before network sync, sends cached cursors to `conversation_sync_since`, and writes after sync/send/push/select/action/history-page. |
| `client/src/app_desktop/desktop_chat_store.h/.cpp` | **C2/C6c/C3e + timeline/search/message actions.** GUI-facing local conversation/message store. Applies full `conversation_sync`, incremental sync, bounded older-history pages, local pending/sent/failed text and attachment send states and push envelopes (`message_deliver`, `message_edited`, `message_deleted`, `message_read_update`, `message_reaction_updated`, `message_pin_updated`), tracks selected conversation, unread counts for unselected conversations, read markers, per-conversation `last_message_id` plus `sync_version` cursors, history-page `next_before_message_id` / `has_more`, and conversation-level pinned/archived/muted flags for folder filtering and quick actions. It retains timestamps, reply/forward metadata, reaction summary, pinned state, silent/scheduled flags, attachment IDs plus filename/mime/size/preview metadata for local sends/cache reloads, supports local conversation filtering and selected-conversation message search, feeds the `BubbleMessageModel` painter roles plus testable text/HTML snapshots with match/focused markers, and can save/load its JSON cache. Cached state displays first, then incremental sync merges missed messages, applies edit/delete/read-marker/reaction/pin deltas and updates title/participant metadata from the server. |
...
| `validate_tdesktop_parity_backend.py` | Runtime control-plane validator for tdesktop-parity backend contracts: contact edit/share/delete, report reason validation, leave confirmation and shared-media provider pagination. |
| `validate_tdesktop_account_domains.py` | Runtime control-plane validator for account-domain settings/features: notification/privacy/security/proxy preferences and Premium/Wallet/Stories/Emoji Status/Gift state. |
...
| `validate_desktop_composer_interactions.py` | Static P1 desktop parity validator for composer-side interactions: retained send-as UI state, compact composer status line, emoji/sticker preview and recent tokens, and preservation of silent/scheduled backend send paths. |
| `validate_desktop_message_interactions.py` | Static P1 desktop parity validator for message-surface interactions: bubble hover action hit-test, quick reaction selector wiring, attachment save/copy actions from the message menu, and inline composer edit submission. |
| `validate_desktop_sidebar_interactions.py` | Static P1 desktop parity validator for left-column and drawer interactions: folder tab counts, row folder routing, mark-as-read, drawer account expansion, archive count and animated Night Mode toggle. |
| `validate_desktop_button_responses.py` | Static desktop button-response validator for visible drawer rows, modal tool buttons, proxy/help controls and right-panel leave/report/gift actions. |
| `validate_desktop_right_panel.py` | Static tdesktop right-column validator for clean Profile section navigation into Media/Members, user contact actions, group member search/profile media and channel subscribers/discussion/similar-channel rows. |
| `validate_desktop_tdesktop_parity_functions.py` | Static desktop validator for the current tdesktop parity batch: compact Profile modal, peer-list Contacts actions, right-panel report/leave/shared-media routing and typed client RPC exposure. |
| `validate_desktop_account_domain_wiring.py` | Static desktop validator for Settings/Gift/Wallet/Stories/Emoji Status account-domain RPC wiring. |
| `validate_desktop_service_interactions.py` | Static desktop parity validator for service/bot right-panel branch and command rows, settings-row routing into real pages/proxy dialog, typed service commands, command suggestions, and actionable shared media rows plus context menus. |
| `validate_service_commands.py` | Runtime control-plane validator for service/bot commands: `/start`, `/help`, `/security`, unknown-command guidance, empty-command rejection, and sync visibility for stored command/reply history. |
...
|---|---|
| POSIX compile path | Code structurally correct; CI now builds Linux + macOS + Windows + Android on every push (M89/M113/M130). |
| C++ client parity with E-series | `app_chat` covers send / read / edit / delete / contacts / presence. `app_desktop` covers registration, profile update, user discovery, contacts, group creation/member management, attachments + image preview, byte-level chunked upload progress, **Remote control + Call settings pages** (M108/M128). |
| AuthService / ChatService callsite refactor for transactional PG writes | Per-entity `upsert_user` / `upsert_session` / `delete_session` / `upsert_remote_session` methods exist on PostgresStateRepository (M129) but the legacy snapshot `save()` is still called by the dispatch path. Wiring AuthService and ChatService to call the per-entity methods on every write is a focused future milestone. |
| UI layer breadth (Android / iOS / Web) | Android (Qt Quick QML, M85) + Web (HTML/JS browser client, M110/M123/M124) shipped. iOS scaffold (Info.plist + CMake guard) ready for a real macOS host (PA-010). Mobile QML now covers Settings hub + Profile / Contacts / Devices / Search / Attachments / RemoteControl / Call (M120-M122) plus the same account settings/features RPC bridge used by desktop for notifications, privacy, security, proxy, Premium/Wallet/Gifts/Stories and Emoji Status. The browser client now keeps its 3-pane shape but adds a right info/settings panel, shared-media pagination and message-row reaction/pin/edit/delete actions over the same WebSocket control-plane envelopes. |
| Desktop visual reference gates | `validate_desktop_gui_smoke.py` remains a functional GUI smoke and screenshot collector. Pixel/reference-map drift is reported by `validate_desktop_image_diff.py` and `validate_desktop_reference_map.py`, but broad sweeps keep those visual comparisons non-blocking unless `TELEGRAM_LIKE_STRICT_IMAGE_DIFF=1` or `TELEGRAM_LIKE_STRICT_REFERENCE_MAP=1` is set for deliberate 1:1 baseline calibration. |
| Real audio / video codecs + capture / playback | Signaling FSM + AEAD-sealed audio frame transport ship today (M109/M127/M128); `PassThroughCodec` is the default and `OpusCodec` is a stub gated on PA-012 (libopus / opuslib). Real platform capture / playback (CoreAudio / WASAPI / ALSA / AAudio) is the next deliverable when PA-012 lands. |
| Designer-supplied PWA icon assets | The browser client (M110/M124) ships an installable PWA manifest + service worker, but `/icons/*` paths are intentionally not yet served — a designer asset bundle is the next deliverable. |
| Product-grade desktop UI polish | Telegram-style 3-pane shell + side menu + 8-page Settings + Remote + Call panels + right info reference states + `BubbleListView` painted message timeline + composer send-as/options/drop-zone controls + design-token palette + real image preview (M125) + byte-level upload progress (M126) ship. Remaining polish: deeper reaction selector/sticker panel behavior, advanced conflict recovery metadata, bot/service-specific right-column variants and CSS-style design system coverage for every QSS literal. |
