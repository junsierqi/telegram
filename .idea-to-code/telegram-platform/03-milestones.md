...
- Delivered: Mobile (M120-M122): MobileChatBridge gains 17 Q_INVOKABLE methods + 8 result signals (profile, contacts, devices, user/message search, attachments, remote-control, voice/video calls). 8 new QML pages (Settings/Profile/Contacts/Devices/RemoteControl/Search/Attachments/Call) wired into Main.qml StackView via ChatListPage settings ⚙ button. C++ ControlPlaneClient gains call_invite/accept/decline/end + CallStateResult struct. Web (M123-M124): single-page client refactored to 3-pane (sidebar with chat list, center pane with selected chat, composer with attachment upload). Service worker /sw.js + /manifest.webmanifest registered for PWA push. Call dialog with invite/accept/decline/end. Desktop polish (M125-M126): design_tokens.h centralizes palette + spacing constants, telegram_stylesheet() consumes them. Real image preview dialog after attachment save (QPixmap.loadFromData). New ControlPlaneClient::send_attachment_chunked() drives ATTACHMENT_UPLOAD_INIT/CHUNK/COMPLETE in sequence with bytes_uploaded/bytes_total progress callback; desktop send path auto-uses chunked for files >1 MB so the progress bar shows real byte progress. Voice/video (M127-M128): server/server/media_codec.py (PassThroughCodec default + OpusCodec stub gated on PA-012; build/parse_audio_payload header), server/server/media_io.py (AudioSource/Sink Protocol + SyntheticAudioSource sine-wave + MemoryAudioSink). Desktop adds Call settings page (Invite/Accept/Decline/End + log). PG transactional (M129): PostgresStateRepository gains upsert_user / upsert_session / delete_session / upsert_remote_session per-entity transactional methods using ON CONFLICT DO UPDATE; remote_sessions table schema + idempotent ALTER TABLE migration for relay_key_b64 (PG parity with M106 SQLite). CI (M130): .github/workflows/ci.yml gains windows-build (windows-latest + Qt6 via aqtinstall + cmake + tests + sweep) and android-build (ubuntu + Qt for Android via aqtinstall + APK upload artifact).
...
- Next: M132 — push GitHub: M120-M131 squashed commit so the new windows-build + android-build CI jobs run. Optional follow-ups: PA-012 procurement (libopus / opuslib) to flip OpusCodec real path; QML smoke runner once a multi-client harness exists; finalize the bundle if user accepts current scope.
...
- Delivered: Post-M131 audit caught four real omissions: (1) server/web/app.js read p.content but the server payload field is content_b64, so browser attachment download silently decoded nothing; (2) server/web/sw.js + manifest.webmanifest referenced /icons/icon-*.png + /icons/badge.png that don't exist + aren't routed by web_bridge.py — install / push notification would 404; (3) README.md still claimed ~45 validators / 4 CI jobs / ReliableChannel deferred / D9 deferred / mobile UI minimal / no voice-video / no web client / no Redis cache; (4) docs/architecture/current-state.md known-gaps section listed several gaps that have since closed (D9, ReliableChannel, mobile UI breadth, web client). Fixes: (1) app.js now reads p.content_b64. (2) sw.js + manifest drop the icon refs (browsers fall back to default app icon — note added saying designer asset bundle is the next deliverable). (3) Comprehensive README rewrite covering M97-M131: every shipped capability, the 7 CI jobs, 70 validators with three skip categories (EXTERNAL / NO_BINARY / PLATFORM), 6 PA placeholders (PA-005/008/009/010/011/012), production deploy stack, and the explicit deferred-engineering list. (4) current-state.md known-gaps now reflects shipped state and lists only the genuine remaining items (codec real impl, AuthService/ChatService callsite refactor for M129 upserts, designer PWA icons, desktop polish remainders).
